...
for(i in sample) {
  ...
  if(length(edges$edge1)!=0){
    edges <- read.delim(paste("Results/Network/",chainType,"/edges_",chainType,"_",i,".txt.outcome.txt",sep = ""))
    vertex <- read.delim(paste("Results/Network/",chainType,"/vertex_",chainType,"_",i,".txt",sep = ""))
    ...
    E(net)$width <- 0.4
    E(net)$color <- c("black")
    tiff(paste("Results/Network/",chainType,"/network_",chainType,"_",i,".tiff",sep=""),res=300,h=3000,w=3000)
...
assign(paste0("PAAD.repertoire.diversity.gini_",chainType),merge(PAAD.repertoire.diversity,cluster_gini_IGHV,by="row.names"))
write.csv(get(paste0("PAAD.repertoire.diversity.gini_",chainType)),file=paste0("Results/Network/",chainType,".csv"))
...
plot(get(paste0("cluster_gini_",chainType))[,"cluster_gini"], 
     get(paste0("cluster_gini_",chainType))[,"vertex_gini"],pch=20,ylab = "Gini (Vextex)",xlab = "Gini (Cluster)")
#legend("bottomright",legend=c("Maternal","Fetal"),col=COLOR,pch=20,cex=c(1.2),ncol=2)

#par(fig=c(0,0.8,0.55,1), new=TRUE)
#summary(lm(get(paste0("cluster_gini_",isotype))[,"cluster_gini"]~summary_data$sample))
#boxplot(get(paste0("cluster_gini_",isotype))[,"cluster_gini"]~summary_data$sample,col=COLOR, horizontal=TRUE, axes=FALSE)

#par(fig=c(0.65,1,0,0.8),new=TRUE)
#summary(lm( get(paste0("cluster_gini_",isotype))[,"vertex_gini"]~summary_data$sample))
#boxplot(get(paste0("cluster_gini_",isotype))[,"vertex_gini"]~summary_data$sample, col=COLOR,axes=FALSE)
dev.off()
