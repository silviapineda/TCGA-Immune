...
files <- list.files("Data/Validation_Normal_pancreas/alignments/")
data<-c()
for(i in files) {
  cat(i, "\n")
  t <- read.delim(paste0("Data/Validation_Normal_pancreas/alignments/",i))
  t$sample<-substr(i, 1, nchar(i)-15)
  data <- rbind(data, t)
}

save(data,file="Data/Validation_Normal_pancreas/Validation_Normal_Pancreas.Rdata")
...
totalReads<-read.table("Data/Validation_Normal_pancreas/report/total_reads.txt",sep=";") ##We need to extract this number from the MIXCR report with the python script
id<-match(rownames(reads),totalReads$V1)
reads$totalReads<-totalReads[id,2]

##Total reads
reads$TRD<-as.numeric("0")
...
write.table(data_clonesInference_Ig,file="Data/Validation_Normal_pancreas/data_for_cloneInfered_Ig_PancreasValidation.txt",row.names = F,sep="\t")
write.table(data_clonesInference_TCR,file="Data/Validation_Normal_pancreas//data_for_cloneInfered_TCR_PancreasValidation.txt",row.names = F,sep="\t")

### After passing the nucleotides.py
##Read the clones and merge with the data
nucleotides_Ig<-read.csv("Data/Validation_Normal_pancreas/ClonesInfered_Ig_ValidationNormal.csv")
nucleotides_TCR<-read.csv("Data/Validation_Normal_pancreas/ClonesInfered_TCR_ValidationNormal.csv")
...
reads_filter$TRD<-as.numeric("0")
reads_filter$TRG<-as.numeric("0")
...
clones$TRD<-as.numeric(0)
clones$TRG<-as.numeric(0)
...
for(i in chain_list){
  ...
  sample_chain<-substr(recon_chain$sample_name,87,95)
...
diversity$entropy_recon_TRD<-0
diversity$entropy_recon_TRG<-entropy_recon_TRG
diversity$entropy_recon_TRG<-0
...
Pancreas.Normal.Validation.repertoire.diversity<-cbind(reads,reads_filter,diversity)
Pancreas.Normal.Validation.repertoire.diversity$sample<-rownames(Pancreas.Normal.Validation.repertoire.diversity)
