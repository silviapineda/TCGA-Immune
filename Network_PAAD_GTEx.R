...
###############################################
#Receptor
Obtain_gini_index<-function(data,receptor,PAAD.GTEx.repertoire.diversity){
...
Obtain_gini_index<-function(data,chainType,PAAD.GTEx.repertoire.diversity){
  ...
  for (i in sample){
    print(i)
    assign(paste0("edges",i),read.delim(paste0("Data/PAAD_GTEx/Network/",chainType,"/edges_",chainType,"_",i,".txt")))
    assign(paste0("vertex",i),read.delim(paste0("Data/PAAD_GTEx/Network/",chainType,"/vertex_",chainType,"_",i,".txt")))
    vertex_max[j]<-max(get(paste0("vertex",i))$Freq)
    vertex_gini[j]<-Gini(get(paste0("vertex",i))$Freq)
    cluster_max[j]<-max(table(get(paste0("edges",i))$V_J_lenghCDR3_CloneId))
    clusters[j]<-sum(table(table(get(paste0("edges",i))$V_J_lenghCDR3_CloneId)))
...
PAAD.GTEx.repertoire.diversity.filter<-
  PAAD.GTEx.repertoire.diversity[which(PAAD.GTEx.repertoire.diversity[,paste0("cluster_gini_",chainType)]!=0 &
                                         PAAD.GTEx.repertoire.diversity[,paste0("vertex_gini_",chainType)]!=0 ),]
...
for(i in sample_tumor) {
  ...
  vertex <- read.delim(paste("Data/PAAD_GTEx//Network/",chainType,"/vertex_",chainType,"_",i,".txt",sep = ""))
  if(length(edges$edge1)!=0){
    ...
    E(net)$width <- 0.4
    E(net)$color <- c("black")
    tiff(paste("Results/Network/network_",chainType,"_",i,".tiff",sep=""),res=300,h=3000,w=3000)
