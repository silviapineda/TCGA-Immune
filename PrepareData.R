...
data_clonesInference<-data_full_cdr3[,c("SEQUENCE_ID","sample","nSeqCDR3","CDR3_length","bestVGene","bestJGene","V_J_lenghCDR3")]
write.table(data_clonesInference,file="Data/PAAD/data_for_cloneInfered_PAAD.txt",row.names = F,sep="\t")

### After passing the nucleotides.py
##Read the clones and merge with the data
nucleotides<-read.csv("Data/PAAD/ClonesInfered_PAAD.csv")
...
data_merge$chainType<-substr(data_merge$bestVGene,1,4)
##Clones per chain
